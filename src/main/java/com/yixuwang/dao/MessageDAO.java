...
public interface MessageDAO {
    ...
    @Select({"select ", SELECT_FIELDS, " from ", TABLE_NAME,
            " where conversation_id=#{conversationId} order by created_date asc limit #{offset}, #{limit}"})
    List<Message> getConversationDetail(@Param("conversationId") String conversationId,
                                        @Param("offset") int offset,
                                        @Param("limit") int limit);

    @Select({"select ", INSERT_FIELDS, " , count(id) as cnt from ( select * from ", TABLE_NAME,
    ...
    void updateStatus(@Param("conversationId") String conversationId, @Param("status") int status);
}
